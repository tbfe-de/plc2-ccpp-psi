...
class meassurement {
	bool first;
	const volatile int *const sensor_addr;
	int minimum;
	int maximum;
	int current;
protected:
...
public:
	...
	meassurement() : sensor_addr(0)		{ assert(false); }
	void operator=(const meassurement&)	{ assert(false); }
#endif
};
...
protected:
	...
	virtual int calculate_current(int);
public:
	avg_meassurement(const int *s_addr);
#ifdef CREATE_DUMMIES_FOR_SWIG_WRAPPER
	// the following are never called but must be defined as
	// because SWIG generates code that makes use of them
	avg_meassurement()			{ assert(false); }
	void operator=(const avg_meassurement&)	{ assert(false); }
#endif
};

inline
meassurement::meassurement(const int *s_addr)
	: first(true), sensor_addr(s_addr)
{}
...
inline
avg_meassurement::avg_meassurement(const int *s_addr)
	: meassurement(s_addr)
{}
