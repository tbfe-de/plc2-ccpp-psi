...
OutIt real_unique_copy(InIt beg, InIt end, OutIt dest) {
	...
	return real_unique_copy_helper<ElemType>(beg, end, dest);
	// or: real_unique_copy_helper<typename InIt::value_type>(beg, end, dest);
	// ... without the typedef - probably a little (or much?) less readable
}
